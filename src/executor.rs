...
impl Executor {
    pub fn new() -> Self {
        Self {
            ...
            reactor: Rc::new(RefCell::new(Reactor::new(16).unwrap())),
        ...
        T: Future<Output = O> + 'static,
    {
        ...
        EX.set(self, || {
            ...
            loop {
                ...
                // block for io
                self.reactor.borrow_mut().wait().unwrap();
                self.reactor.borrow_mut().dispatch_completions();
            }
        })
    }
}
...
impl TaskQueue {
    ...
    pub(crate) fn push(&self, runnable: Rc<Task>) {
        println!("[executor] add task");
        self.queue.borrow_mut().push_back(runnable);
    }

    pub(crate) fn pop(&self) -> Option<Rc<Task>> {
        let res = self.queue.borrow_mut().pop_front();
        if res.is_some() {
            println!("[executor] remove task");
        }
        res
    }
